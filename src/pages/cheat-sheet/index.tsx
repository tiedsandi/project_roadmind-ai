...
export default function CheatSheetPage() {
  ...
  const handleGenerate = async () => {
    ...
    setLoading(true);
    const res = await fetch("/api/generateCheatSheet", {
      method: "POST",
    ...
    setLoading(false);
    setShowModal(false);
    if (res.ok) router.push(`/cheat-sheet/${data.id}`);
  };

  const handleDelete = async () => {
    if (!deleteId) return;
    setDeleting(true);
    await cheatSheetRepository.delete(deleteId);
    setSheets((prev) => prev.filter((s) => s.id !== deleteId));
    setDeleting(false);
    setDeleteId(null);
  };
  ...
  if (!userId) {
    return (
      <div className="max-w-3xl mx-auto px-6 md:px-16 py-12">
        <h1 className="text-3xl font-bold text-[#37352f] mb-2">Cheat Sheet</h1>
  ...
  return (
    <div className="max-w-3xl mx-auto px-6 md:px-16 py-12">
      ...
      <div className="flex items-center justify-between mb-2">
        <div>
          <h1 className="text-3xl font-bold text-[#37352f]">Cheat Sheet</h1>
          <p className="text-[#9b9a97] mt-1 text-sm">
            Ringkasan 1 halaman untuk review dalam 5 menit
          </p>
        </div>
        <button
        ...
        {sheets.length === 0 ? (
          <div className="py-16 text-center">
            <p className="text-[#9b9a97] text-sm mb-4">
              Belum ada cheat sheet. Buat yang pertama!
            </p>
            <button
              onClick={() => setShowModal(true)}
            ...
            {sheets.map((s) => (
              ...
              >
                ...
                <div
                  ...
                  onClick={() => router.push(`/cheat-sheet/${s.id}`)}
                >
                  ...
                  <p className="text-xs text-[#9b9a97] truncate mt-0.5">
                    {s.konsepUtama.length} konsep · {s.tips.length} tips ·{" "}
                    {s.kesalahanUmum.length} kesalahan umum
                  </p>
                </div>
                <button
